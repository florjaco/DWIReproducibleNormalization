...
# Normalization of FA Templates to MNI152_T1_1mm template. FA and MD are automatically coregistered because they are calculated at the same time.
...
(
	...
	echo "Normalizing ${subj}_FA_template0 (FA template) to MNI152_T1_1mm_brain"
	echo `date`		

		MOVING=${subj}_FA_template0.nii.gz #FA is the Moving image
		...
		f=$FIXED ; m=$MOVING    # fixed and moving image file names		
...
$reg --dimensionality $dim --float 0 --verbose 1 \
			...
			--collapse-output-transforms 1 \
                        --output [${nm},${nm}_${m}_warped.nii.gz,${nm}_inv.nii.gz]
